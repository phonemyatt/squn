...
describe("mapping/nested-mapper — splitAndMap()", () => {
  it("splits rows at splitOn boundary into nested objects", () => {
    ...
    const result = splitAndMap(rows, ["role_id"], [identity, identity]) as Record<string, unknown>[];
  ...
  it("handles multiple rows with different primary keys", () => {
    ...
    const result = splitAndMap(rows, ["dept_id"], [identity, identity]) as Record<string, unknown>[];
