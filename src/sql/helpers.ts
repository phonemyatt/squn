...
export function sqlJoin(fragments: readonly SqlFragment[], separator: string = ", "): SqlFragment {
  ...
  for (let i = 0; i < fragments.length; i++) {
    ...
    const renumbered = f.text.replace(/\$(\d+)/g, (_, n: string) => `$${Number.parseInt(n, 10) + offset}`);
...
export function sqlRaw(text: string): SqlFragment {
  ...
  if (SQUN_REGEX.NULL_BYTE.test(text)) {
    throw new SecurityError(ErrorCode.INJECTION_DETECTED, "Null byte detected in sqlRaw()", { operation: "sqlRaw", sql: text });
  }
  if (SQUN_REGEX.STACKED_STATEMENTS.test(text)) {
    throw new SecurityError(ErrorCode.INJECTION_DETECTED, "Stacked statements detected in sqlRaw()", { operation: "sqlRaw", sql: text });
  }
  if (SQUN_REGEX.MSSQL_DANGEROUS.test(text)) {
    throw new SecurityError(ErrorCode.INJECTION_DETECTED, "Dangerous MSSQL procedure detected in sqlRaw()", { operation: "sqlRaw", sql: text });
  }
  // Check high patterns
  if (SQUN_REGEX.UNION_INJECTION.test(text)) {
    throw new SecurityError(ErrorCode.INJECTION_DETECTED, "UNION injection detected in sqlRaw()", { operation: "sqlRaw", sql: text });
  }
  if (SQUN_REGEX.TAUTOLOGY.test(text)) {
    throw new SecurityError(ErrorCode.INJECTION_DETECTED, "Tautology detected in sqlRaw()", { operation: "sqlRaw", sql: text });
  }
  if (SQUN_REGEX.TIME_BASED.test(text)) {
    throw new SecurityError(ErrorCode.INJECTION_DETECTED, "Time-based injection detected in sqlRaw()", { operation: "sqlRaw", sql: text });
  }
  if (SQUN_REGEX.MYSQL_DANGEROUS.test(text)) {
    throw new SecurityError(ErrorCode.INJECTION_DETECTED, "Dangerous MySQL operation detected in sqlRaw()", { operation: "sqlRaw", sql: text });
  }
  if (SQUN_REGEX.PG_DANGEROUS.test(text)) {
    throw new SecurityError(ErrorCode.INJECTION_DETECTED, "Dangerous PostgreSQL function detected in sqlRaw()", { operation: "sqlRaw", sql: text });
  }
...
export function sqlIdentifier(name: string): SqlFragment {
  if (name.length === 0) {
    throw new SecurityError(ErrorCode.INVALID_IDENTIFIER, "Identifier must not be empty", { operation: "sqlIdentifier" });
  }
  if (name.length > 128) {
    throw new SecurityError(ErrorCode.INVALID_IDENTIFIER, `Identifier exceeds 128 characters: ${name.length}`, { operation: "sqlIdentifier" });
  }
  if (!SQUN_REGEX.VALID_IDENTIFIER.test(name)) {
    const invalid = [...name].filter((c) => !/[a-zA-Z0-9_]/.test(c));
    throw new SecurityError(ErrorCode.INVALID_IDENTIFIER, `Invalid identifier characters: ${invalid.join(", ")}`, { operation: "sqlIdentifier" });
  }
